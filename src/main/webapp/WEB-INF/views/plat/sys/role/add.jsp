...
<div class="page-content">
    <div class="page-header">
        <h1>
            角色管理
            <small><i class="ace-icon fa fa-angle-double-right"></i> 新增角色
            </small>
        </h1>
    </div>
    <!-- /.page-header -->
        ...
        <div class="col-xs-12">
            <form id="roleForm" name="roleForm" class="form-horizontal" action="add.htm" method="post">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <input id="name" name="name" type="text" class="col-xs-10 col-sm-5" placeholder="" value="">
                                    ...
                                    <div class="widget-header">
                                        <h4 class="widget-title">
                                            <label> <input name="module" class="ace father" type="checkbox" id="mod_${sysModule.id }"
                                                           value="${sysModule.id }" /> <span class="lbl"> <i class="ace-icon fa fa-pencil-square-o"></i> ${sysModule.name }</span>
                                            </label>
                                        </h4>
                                            ...
                                            <c:forEach items="${listModule }" var="secModule" varStatus="st">
                                                <c:if test="${sysModule.id eq secModule.parentId }">
                                                    <label> <input name="module" id="mod_${secModule.id }" type="checkbox" value="${secModule.id }"
                                                                   class="ace children" /> <span class="lbl"> <i class="ace-icon fa fa-pencil-square-o"></i> ${secModule.name}</span>
                                                    </label>
                                                    <c:forEach items="${listFunction }" var="sysFunction" varStatus="st">
                                                        <c:if test="${sysFunction.parentId eq secModule.id }">
                                                            <label class="checkbox inline" style="color: blue"><input name="function" id="function_${sysFunction.id }"
                                                                                                                      type="checkbox" value="${sysFunction.id }" class="ace children" />
                                                                <span class="lbl"> <i class="ace-icon fa fa-align-justify"></i> ${sysFunction.name }</span> </label>
                                                        </c:if>
                                                    </c:forEach>
                                                    <br />
                                                </c:if>
